...
class MongoDBCacheBackend(BaseCache):
    def __init__(self, host="localhost", port=27017, db_name="cache_db", collection_name="cache", **kwargs):
        ...
        self.loop.create_task(self._create_index())  # 异步创建索引

    async def _create_index(self):
        await self.collection.create_index("expires_at", expireAfterSeconds=0)
        await self.collection.create_index("key", unique=True)

    async def _async_set(self, key, value, timeout=None, version=None):
        key = self.make_key(key, version)
        timeout = self.get_backend_timeout(timeout)
        expires_at = datetime.utcnow() + timedelta(seconds=timeout) if timeout else None
        try:
            await self.collection.update_one(
                {"key": key},
                {"$set": {"value": Binary(force_bytes(value)), "expires_at": expires_at}},
                upsert=True,
            )
        except PyMongoError as e:
            print(f"Set operation failed: {e}")
    ...
    def set(self, key, value, timeout=None, version=None):
        return self.loop.run_until_complete(self._async_set(key, value, timeout, version))

    async def _async_get(self, key, default=None, version=None):
        key = self.make_key(key, version)
        try:
            document = await self.collection.find_one({"key": key})
            if document:
    ...
    def get(self, key, default=None, version=None):
        return self.loop.run_until_complete(self._async_get(key, default, version))
